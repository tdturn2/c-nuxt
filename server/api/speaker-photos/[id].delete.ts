...
export default defineEventHandler(async (event) => {
  ...
  if (!payloadBaseUrl) throw createError({ statusCode: 500, statusMessage: 'Missing PAYLOAD_BASE_URL' })
  ...
  if (!token && !getHeader(event, 'cookie')) {
    throw createError({
      statusCode: 401,
      statusMessage: 'No Payload credentials to forward (missing connect-users token and Cookie header).',
    })
  }

  const url = `${origin}/api/speaker-photos/${encodeURIComponent(id)}`
  ...
  if (!res.ok) {
    throw createError({
      ...
      statusMessage: res.statusText || 'Failed to delete speaker-photos',
      data: json,
    })
  }
