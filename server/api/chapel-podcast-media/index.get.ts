...
export default defineEventHandler(async (event) => {
  ...
  const url = `${payloadBaseUrl}/api/chapel-podcast-media?${searchParams.toString()}`
  ...
  try {
    ...
    throw createError({
      statusCode,
      statusMessage: err?.statusMessage || err?.message || 'Failed to fetch chapel podcast media',
      data: err?.data,
    })
  }
})
