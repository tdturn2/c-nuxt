        ...
        <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          <div class="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <USelectMenu
                v-model="parentPageSelection"
                :items="parentPageSelectItems"
                value-key="value"
                label-key="label"
                color="neutral"
                variant="outline"
                placeholder="None (top-level)"
                :search-input="{ placeholder: 'Search pages…', icon: 'i-lucide-search' }"
                class="w-full"
              />
            </div>
            <div>
...
const props = withDefaults(defineProps<{
  ...
  pages?: ConnectPage[]
  sectionOptions?: Array<{ id: string | number; label: string }>
  canManage?: boolean
}>(), {
...
// --- Internal pages (when pages prop omitted) ---
const internalPages = ref<ConnectPage[]>([])
let pagesLoadPromise: Promise<void> | null = null

async function ensurePagesLoaded() {
  if (props.pages) return
  if (internalPages.value.length) return
  if (!pagesLoadPromise) {
    pagesLoadPromise = (async () => {
      try {
        const res = await fetchAllConnectPages({
          limit: 100,
          depth: 2,
          sort: '-updatedAt',
        })
        internalPages.value = Array.isArray(res?.docs) ? res.docs : []
      } catch {
        // Parent dropdown may be empty; openEditById can still fetch by id.
      }
    })().finally(() => {
...
// --- Section options (when prop omitted) ---
const connectUserFetch = useFetch<any>('/api/connect-users/me', {
  key: 'connect-page-edit-slideover-connect-user',
  immediate: false,
})

const internalSectionOptions = computed(() => {
  const doc = connectUserFetch.data.value?.doc ?? null
  const editable = doc?.editableSections ?? doc?.fields?.editableSections
...
const sectionOptions = computed(() => {
  if (props.sectionOptions) return props.sectionOptions
  return internalSectionOptions.value
})
...
const {
  ...
  key: 'connect-page-edit-slideover-employees',
  immediate: false,
})
...
const {
  ...
  key: 'connect-page-edit-slideover-connect-pages-media',
  immediate: false,
  query: () => ({
    limit: 50,
    sort: '-createdAt',
...
function runTableCommand(command: string, attrs?: Record<string, unknown>) {
  const inst = docsBodyUEditorRef.value as any
  const editor = inst?.editor != null ? unref(inst.editor) : null
  if (!editor || editor.isDestroyed) return
  const chain = editor.chain().focus()
  const fn = chain[command]
  if (typeof fn === 'function') fn.call(chain, attrs).run()
}
  ...
  [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Add row above', icon: 'i-lucide-rows-3', onClick: () => runTableCommand('addRowBefore') },
        { label: 'Add row below', icon: 'i-lucide-rows-3', onClick: () => runTableCommand('addRowAfter') },
        { label: 'Add column before', icon: 'i-lucide-columns-3', onClick: () => runTableCommand('addColumnBefore') },
        { label: 'Add column after', icon: 'i-lucide-columns-3', onClick: () => runTableCommand('addColumnAfter') },
        { label: 'Delete row', icon: 'i-lucide-rows-2', onClick: () => runTableCommand('deleteRow') },
        { label: 'Delete column', icon: 'i-lucide-columns-2', onClick: () => runTableCommand('deleteColumn') },
        { label: 'Merge selected cells', icon: 'i-lucide-panels-top-left', onClick: () => runTableCommand('mergeCells') },
        { label: 'Split cell', icon: 'i-lucide-grid-2x2', onClick: () => runTableCommand('splitCell') },
        { label: 'Toggle header row', icon: 'i-lucide-panel-top', onClick: () => runTableCommand('toggleHeaderRow') },
        { label: 'Delete table', icon: 'i-lucide-trash-2', onClick: () => runTableCommand('deleteTable') },
      ],
    },
...
watch(editorOpen, (open) => {
  if (!open) return
  if (props.canManage) refreshConnectPagesMedia()
  if (!props.sectionOptions) connectUserFetch.execute()
  refreshEmployees()
...
/** Remove `edit` / `create` from the URL so the route watch does not reopen the slide-over after refresh. */
...
async function openEditById(id: string | number) {
  const sid = String(id).trim()
  if (!sid) return
  await ensurePagesLoaded()
  let p = pages.value.find((x) => String(x.id) === sid)
  if (!p) {
    try {
      const full = await $fetch<any>(`/api/connect-pages/${encodeURIComponent(sid)}`, {
        query: { depth: 2 },
      })
      p = (full?.doc ?? full) as ConnectPage
    } catch (err) {
      console.error('[ConnectPageEditSlideover] openEditById failed', err)
      return
    }
  }
  if (p?.id != null) await openEdit(p)
}

const parentPageOptions = computed(() => (
  pages.value
    .filter((p) => String(p.id) !== String(editingId.value ?? ''))
    .map((p) => ({
      id: p.id,
...
/** USelectMenu cannot round-trip an empty string, so "no parent" needs its own sentinel. */
const PARENT_PAGE_NONE = '__none__'

const parentPageSelectItems = computed(() => [
  { label: 'None (top-level)', value: PARENT_PAGE_NONE },
  ...parentPageOptions.value.map((opt) => ({ label: opt.title, value: String(opt.id) })),
])

const parentPageSelection = computed({
  get: () => form.value.parentId || PARENT_PAGE_NONE,
  set: (value: string) => {
    form.value.parentId = value === PARENT_PAGE_NONE ? '' : value
  },
})
...
function lexicalToTipTap(lexical: any): any {
  ...
  const convert = (node: any): any[] => {
    ...
    if (node.type === 'tablecell' || node.type === 'tableCell') {
      const converted = (node.children || []).flatMap(convert)
      ...
      const content = !converted.length
        ? [{ type: 'paragraph', content: [] }]
        : converted.every((child: TipTapNode) => blockTypes.has(child.type))
          ? converted
          : [{ type: 'paragraph', content: converted }]
      ...
      return [{
        type: headerState > 0 ? 'tableHeader' : 'tableCell',
        attrs: {
          colspan: colSpan,
          rowspan: rowSpan,
          colwidth: null,
...
async function deletePage() {
  if (editingId.value == null || editingId.value === '') return
...
function syncDocsBodyEditorFromState(): boolean {
  ...
  try {
    ...
    console.error('[ConnectPageEditSlideover] TipTap setContent failed', err)
    return false
  }
}
...
.connect-docs-editor .tableWrapper {
  margin: 1rem 0;
  overflow-x: auto;
}

.connect-docs-editor table {
  width: 100%;
  min-width: 32rem;
...
.connect-docs-editor th,
.connect-docs-editor td {
  position: relative;
  min-width: 6rem;
...
.connect-docs-editor th > p,
.connect-docs-editor td > p {
  margin: 0;
}

.connect-docs-editor .selectedCell::after {
  position: absolute;
  inset: 0;
  z-index: 2;
  content: '';
  pointer-events: none;
  background: rgba(13, 94, 130, 0.14);
}

.connect-docs-editor .column-resize-handle {
  position: absolute;
  top: 0;
  right: -2px;
  bottom: -2px;
  width: 4px;
  pointer-events: none;
  background: rgba(13, 94, 130, 0.75);
}
