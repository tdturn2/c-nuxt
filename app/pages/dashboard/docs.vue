          ...
          <details open class="rounded-lg border border-gray-200 bg-white shadow-sm [&_summary::-webkit-details-marker]:hidden">
            ...
            <div class="mt-3 space-y-2">
              <details class="rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
                <summary class="cursor-pointer text-xs font-semibold text-gray-700">Special blocks in content (videos &amp; FAQ)</summary>
  ...
  try {
    if (editingId.value) {
      await $fetch(`/api/connect-pages-update/${editingId.value}`, {
        method: 'PATCH',
        body: { title: form.value.title.trim(), slug: form.value.slug.trim(), parent, navCategory, content, contactsHeading: contactsHeadingValue, contacts },
      })
    } else {
      const created = await $fetch<any>('/api/connect-pages', {
        method: 'POST',
        body: { title: form.value.title.trim(), slug: form.value.slug.trim(), parent, navCategory, section, content, contactsHeading: contactsHeadingValue, contacts },
      })
