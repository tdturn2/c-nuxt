...
/** True when the given path belongs to the departments tree (category landing or department page). */
export function isConnectDepartmentsPath(rawPages: any[], path: string): boolean {
  const normalizedPath = normalizeConnectPageLookupPath(path)
  if (normalizedPath === '/internal' || normalizedPath.startsWith('/internal/')) return true
  const isCategoryLanding = CONNECT_PAGE_CATEGORIES.some(
    (c) => normalizeConnectPageLookupPath(CONNECT_PAGE_CATEGORY_PATHS[c.value]) === normalizedPath,
  )
  if (isCategoryLanding) return true
  return resolveNavCategoryForPath(rawPages, path) != null
}
...
/** Departments sidebar: always the full category tree so the current page keeps its context. */
